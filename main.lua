...
-- weight model
cmd:option('-use_weight', 0, 'should we capture the weight info?')
...
cmd:option('-m2_batch_size', 100, 'number of sequences to train on in parallel')
cmd:option('-use_hsm', 0, 'use hierarchical softmax (yes = 1) or inefficient full softmax')
cmd:option('-hsm_bins', 100, 'hsm bin size')
cmd:option('-m2_neg_samples', 15, 'no. of negative samples for each node')
-- label model
...
cmd:option('-max_epochs', 25000, 'number of full passes through the training data')
cmd:option('-gpu', 1, '1=use gpu; 0=use cpu;')
cmd:option('-params_init', 0.25, 'initialize the parameters of the model from a uniform distribution')
...
cmd:option('-use_decay', 1, '0 = std; 1 = paper;')
cmd:option('-decay_rate', 0.005, 'decay rate')
cmd:option('-save_every', 5, 'save model every <int> epochs')
